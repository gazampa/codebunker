...
alias dxrt='sfdx force:apex:test:run --classnames GBE_WorkOrderCreateWSEventLogTest'
alias dxrs='sfdx force:apex:test:run --tests "GBE_WorkOrderHelper_Test.updateEmergencyCase_Test" -r human'
alias dxrsu='sfdx force:apex:test:run -s "GBE_WorkOrders"'
alias dxrtm='sfdx force:apex:test:run -t "GBE_WorkOrderHelper_Test.updateEmergencyCase_Test"'
...
alias tra='tar -rvf'
alias trv='tar tvfW'
alias passwd1='openssl rand -base64 64'
alias passwd2="tr -dc '[:graph:]' < /dev/urandom | head -c 1000"
alias passwd3='source ~/scripts/random.sh'
alias run='source '
...
rand()
{
  sudo < /dev/urandom tr -dc 'A-Za-z0-9' | head -c $1;
  printf '\n';
}
