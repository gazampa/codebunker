...
import java.net.*;
import java.util.*;
class BasicWebServer implements HttpConstants {
...
class Worker extends BasicWebServer implements HttpConstants, Runnable {
    ...
    public synchronized void run() {
        while(true) {
             ...
             */
            s = null;
            Vector pool = BasicWebServer.threads;
            synchronized (pool) {
                if (pool.size() >= BasicWebServer.workers) {
    ...
    void handleClient() throws IOException {
         ...
         */
        s.setSoTimeout(BasicWebServer.timeout);
...
outerloop:
            ...
            File targ = new File(BasicWebServer.root, fname);
            if (targ.isDirectory()) {
