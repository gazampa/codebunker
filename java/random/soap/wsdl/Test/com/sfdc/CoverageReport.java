...
public class CoverageReport {
	...
	public void createCoverageReport(ClassCoverageResult[] results,
		...
		HSSFCell numLocationsCell = headerRow.createCell(2);
		numLocationsCell.setCellValue(Constants.CLASS_NUM_LOCATIONS_COLUMN_HEADER);
		numLocationsCell.setCellStyle(style);
		...
		if (results != null && results.length > 0) {
			...
			for (int i = 1; i <= results.length; i++) {
				...
				dataRow.createCell(1).setCellValue(result.getCoveragePercentage());
				dataRow.createCell(2).setCellValue(result.getNumLocations());
			}
		}
