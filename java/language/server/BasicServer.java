...
import java.net.*;
import java.util.*;
class BasicServer implements HttpConstants {
    ...
    protected final static void log(String s) {
        synchronized (log) {
            log.println(s);
            log.flush();
        }
    ...
    public static void main(String[] a) {  try{
        ...
        ServerSocket ss = new ServerSocket(port); // Instantiates a server socket and goes into application loop.
        while (true) {

            Socket s = ss.accept();

            Worker w = null;
            synchronized (threads) { // Access to pool is synchronized
                if (threads	.isEmpty()) {
                    ...
                    threads.removeElementAt(0);
          			p( "Server Socket : " + ss + " Worker Socket : " + s );
                    w.setSocket(s);
                }
            }
        }	}catch(Exception e){log("Exception in main" + e);}
    }
}


class Worker extends BasicServer implements HttpConstants, Runnable {
    ...
    public synchronized void run() {
        while(true) {
             ...
             */
            s = null;
            Vector pool = BasicServer.threads;
            synchronized (pool) {
                if (pool.size() >= BasicServer.workers) {
    ...
    void handleClient() throws IOException {
         ...
         */
        s.setSoTimeout(BasicServer.timeout);
...
outerloop:
			...
			String string = new String(buf);p(string);
            ...
            File targ = new File(BasicServer.root, fname);
            if (targ.isDirectory()) {
